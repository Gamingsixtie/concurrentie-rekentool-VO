...
type JourneyEntryType = 'conversation' | 'blokkade' | 'status_change' | 'first_contact';
...
export default function CustomerJourneyTimeline({
  ...
  const [showBlockadeForm, setShowBlockadeForm] = useState(false);
  ...
  if (entries.length === 0 && !showBlockadeForm) {
    return (
      <div className="bg-white border border-neutral-200 rounded-lg p-8 text-center">
        ...
        <p className="text-[14px] text-neutral-500">
          De klantreis wordt opgebouwd uit gesprekken en statuswijzigingen. Voeg een gesprek toe of wijzig een engagement-status om te beginnen.
        </p>
        <button
          type="button"
          onClick={() => setShowBlockadeForm(true)}
          className="mt-4 text-[14px] font-semibold text-cito-accent hover:text-orange-700"
          aria-expanded={showBlockadeForm}
        >
        ...
        {showBlockadeForm && (
          <BlockadeForm
            schoolId={schoolId}
            contacts={contacts}
            onSaved={() => setShowBlockadeForm(false)}
            onCancel={() => setShowBlockadeForm(false)}
          />
        )}
      </div>
    );
